...
class BPSClassifier(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Using the torchmetrics Accuracy class for accuracy
        # Because it automatically thresholds the predictions
        acc = self.val_acc(y_pred, y)
        
        # the following code gave some weird results
        # val_acc = torch.mean((torch.eq(y_pred, y)).float())



        # self.log('val_loss', val_loss)          # Tensorboard
        wandb.log({'val_loss' : val_loss, 'val_acc' : acc})      # Weights and Biases
