...
def perform_tsne(X_reduced_dim: np.ndarray,
    ...
    tsne = TSNE(n_components=n_components, perplexity=perplexity, learning_rate=lr, 
                verbose=verbose, angle=angle)
...
def main():
    ...
    # transformed_pca = TSNE.fit_transform(tsne_df_pca)
    # print(f'X_tsne_pca.shape: {transformed_pca.shape}')
    
    tsne_df_direct = create_tsne_cp_df(X_tsne_direct.fit_transform(image_stream_1d), all_labels, 1000)
    # print(tsne_df_direct.head())
    # transformed_direct = TSNE.fit_transform(tsne_df_direct)
    # print(f'tsne_df_direct.shape: {transformed_direct.shape}')
    tsne_df_pca = create_tsne_cp_df(X_tsne_pca.fit_transform(X_pca), all_labels, 1000)
    # print(tsne_df_pca.head())
    # transformed_head = TSNE.fit_transform(tsne_df_pca)
    # print(f'tsne_df_pca.shape: {transformed_head.shape}')
