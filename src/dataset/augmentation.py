...
class NormalizeBPS(object):
    def __call__(self, img_array) -> np.array(np.float32):
        ...
        """
        norm = (img_array-np.min(img_array))/65535
...
class ResizeBPS(object):
    ...
    def __call__(self, img:np.ndarray) -> np.ndarray:
        ...
        """
        img_resize = cv2.resize(img, (self.resize_width,self.resize_height), interpolation = cv2.INTER_AREA)
...
class RandomCropBPS(object):
    ...
    def __init__(self, output_height: int, output_width: int):
        self.out_height = output_height
        self.out_width = output_width

    def __call__(self, image):
        toTens = ToTensor()
        imgTensor = toTens(image)
        cropper = torchvision.transforms.RandomCrop((self.out_height, self.out_width))
        return np.squeeze(cropper.forward(imgTensor).numpy(), 0)
...
class ToTensor(object):
    ...
    def __call__(self, image: np.ndarray) -> torch.Tensor:
        ...
        return torch.tensor(np.expand_dims(np.array(image, np.float32), 0))
