...
class BPSMouseDataset(torch.utils.data.Dataset):
    ...
    def __init__(
            ...
            meta_csv_file:str,
            meta_root_dir:str,
            s3_client: boto3.client = boto3.client('s3', config=Config(signature_version=UNSIGNED)),
        ...
        csv_path = os.path.join(meta_root_dir, meta_csv_file)
        ...
        if not os.path.exists(csv_path):
            Bio = get_bytesio_from_s3(s3_client, bucket_name, f"Microscopy/train/meta.csv")
            self.meta_df = pd.read_csv(Bio)
        else:
            try:
    ...
    def __getitem__(self, idx):
        ...
        Read BytesIO using Numpy (np frombuffer, let it typecast to uint8), then imread using cv2 -- pass in second arg: set ANYDEPTH (brings it back to uint16)
        """
        ...
        imgpath = os.path.join(self.root_dir, filename)
        ...
        if self.on_prem:
            try:
                ...
                if img:
                    array_form = np.array(img, np.uint16)
            except:
                failed = True

        if not self.on_prem or failed:
            imgpath = f"Microscopy/train/{filename}"
            Bio = get_bytesio_from_s3(self.client, self.bucket_name, imgpath)
            # numpy array, unsigned 16-bit integers
            img = Image.open(Bio)
            array_form = np.array(img, np.uint16)
        ...
        result = array_form
        if self.transform:
            result = self.transform(array_form)

        # return the image and associated label
        return (result, row["particle_type"])
