...
class BPSDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        test_path = f"{self.test_dir}/{self.test_csv}"
        train_path = f"{self.train_dir}/{self.train_csv}"
        val_path = f"{self.val_dir}/{self.val_csv}"

        # call the save_tiffs_local_from_s3 function to download tiffs from the
        # train_csv file.
        save_tiffs_local_from_s3(self.s3_client, self.bucket_name, self.s3_path, train_path, self.train_dir)


        # call the save_tiffs_local_from_s3 function to download tiffs from the
        # val_csv file.        
        save_tiffs_local_from_s3(self.s3_client, self.bucket_name, self.s3_path, val_path, self.val_dir)
        ...
        if self.test_csv is not None:
            ...
            save_tiffs_local_from_s3(self.s3_client, self.bucket_name, self.s3_path, test_path, self.test_dir)
    ...
    def setup(self, stage: str):
        ...
        if stage == "train":
            ...
            self.training_loader = BPSMouseDataset(meta_csv_file=self.train_csv,
        ...
        if stage == "validate":
            ...
            self.validation_loader = BPSMouseDataset(meta_csv_file=self.val_csv,
        ...
        if stage == "test":
            ...
            self.test_loader = BPSMouseDataset(meta_csv_file=self.test_csv,
